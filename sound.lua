...
function set_volume(source, new_volume)
    ...
    if CONFIG_TABLE.debug_mode then log.trace("set_volume called") end
...
function find_character_SFX(character, SFX_sound_name)
    ...
    for k,currentDirectory in ipairs(DIRECTORIES_TO_CHECK) do
      ...
      if SFXSounName == "chain" and current_directory_has_chain_sound then
        if CONFIG_TABLE.debug_mode then log.trace("loaded "..SFX_sound_name.." for "..character) end
      ...
      elseif SFX_sound_name == "combo" and current_directory_has_chain_sound then
        if CONFIG_TABLE.debug_mode then log.trace("substituted found chain SFX for "..SFX_sound_name.." for "..character) end
      ...
      if SFX_sound_name == "chain" and current_directory_has_combo_sound then
        if CONFIG_TABLE.debug_mode then log.trace("substituted found combo SFX for "..SFX_sound_name.." for "..character) end
        return current_directory_has_combo_sound
      end
      ...
      if other_requested_SFX then
        ...
        if CONFIG_TABLE.debug_mode then log.trace("did not find "..SFX_sound_name.." for "..character.." in current directory: "..current_dir) end
      end
      if current_directory_has_chain_sound or current_directory_has_combo_sound --[[and we didn't find the requested SFX in this dir]] then
        if CONFIG_TABLE.debug_mode then log.trace("chain or combo was provided, but "..SFX_sound_name.." was not.") end
...
function find_music(character, music_type)
    ...
    if character_theme_overrides_stage_theme then
      ...
      if found_source then
        if CONFIG_TABLE.debug_mode then log.trace("In selected sound directory, found "..music_type.." for "..character) end
      else
        if CONFIG_TABLE.debug_mode then log.trace("In selected sound directory, did not find "..music_type.." for "..character) end
      end
      return found_source
    else
      if stages[character] then
        ...
        if soundSetOverridesDefaultSoundSet then
          ...
          if found_source then
            if CONFIG_TABLE.debug_mode then log.trace("In selected sound directory stages, found "..music_type.." for "..character) end

          else
            if CONFIG_TABLE.debug_mode then log.trace("In selected sound directory stages, did not find "..music_type.." for "..character) end
          end
          return found_source
        else
          found_source = check_supported_extensions("sounds/"..default_sounds_dir.."/music/"..stages[character].."/"..music_type)
          if found_source then
            if CONFIG_TABLE.debug_mode then log.trace("In default sound directory stages, found "..music_type.." for "..character) end
          else
            if CONFIG_TABLE.debug_mode then log.trace("In default sound directory stages, did not find "..music_type.." for "..character) end
          end
          return found_source
        end
      end
