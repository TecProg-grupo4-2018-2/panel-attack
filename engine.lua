...
function Stack.PdP(self)
	...
	if self.speed_times then
		...
		if ok then
			...
			assert(self.speed > 0)
			ok = self.speed_times.idx ~= #self.speed_times
		elseif ok then
	...
	and not self.rise_lock and self.mode ~= "puzzle" then
		ok = self.panels_in_top_row
		if ok then
			self.health = assert(self.health - 1)
			ok = self.health == 0 and self.shake_time == 0
		end
		if ok then
			self.game_over = true
		end
		if not self.panels_in_top_row then
			...
			ok = self.rise_timer <= 0
		end
		if ok then
			self.displacement = self.displacement - 1
			ok = self.displacement == 0
		end
		if ok then
			self.prevent_manual_raise = false
			self.top_cur_row = assert(self.height)
	...
	local drop_it = self.n_active_panels == 0 and
		...
		(not self.panels_in_top_row or garbage_fits_in_populated_top_row) and
		not self:has_falling_garbage()
